...
async function fetchData() {
  ...
  const datasets = ipList.map(ip => {
    ...
    return {
      label: `IP: ${ip}`,
      data: ipData.map(item => ({ x: item.dateTime ? parseDateTime(item.dateTime) : null, y: item.gpuTemp })),
      lineTension: 0.3,
...
function updateChartData() {
  gpuLineChat.data.labels = GpuDateTimeData;
  gpuLineChat.data.datasets[0].label = 'GPU 온도';
  gpuLineChat.data.datasets[0].data = gpuTempData;
  gpuLineChat.update();
}

var ctx = document.getElementById('GPUChart');
var gpuLineChat = new Chart(ctx, {
  type: 'line',
  data: {
    labels: GpuDateTimeData,
    datasets: [
    {
//      label: 'GPU 온도',
//      lineTension: 0.3,
